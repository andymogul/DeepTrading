...
def Portfolio(env):
    ...
    if os.path.isfile(os.path.join(os.getcwd(), model_name+".index")):
        ...
        first_stock_num = np.empty([217])
        second_stock_num = np.empty([217])
        date_pointer = 0
        while(not done):
            ...
            first_stock_num[date_pointer] = state[2]
            second_stock_num[date_pointer] = state[3]
            portfolio_queue[date_pointer] = state[4]
            date_pointer += 1
        print(date_pointer)
        np.savetxt(model_name+"_act.csv", act_queue, delimiter=",")
        np.savetxt(model_name+"_portfolio.csv", portfolio_queue, delimiter=",")
        np.savetxt(model_name+"_first_stock.csv", first_stock_num, delimiter=",")
        np.savetxt(model_name+"_second_stock.csv", second_stock_num, delimiter=",")
